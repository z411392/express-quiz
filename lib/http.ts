...
import { onDeletingUser } from "@/modules/user-managing/presentation/http/controllers/on-deleting-user"

const router = Router()

router.post("/users", onCreatingUser).get("/users", onListingUsers).delete("/users/:userId", onDeletingUser)
